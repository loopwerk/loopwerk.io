...
It got me wondering: _why_ does this approach feel so right? I think the answer lies in the guiding principles of Python itself: the [Zen of Python](https://en.m.wikipedia.org/wiki/Zen_of_Python). Look at Django's view layer with those principles in mind, and you can see where things go wrong.

Here are all the ways that Django's views (especially the generic class-based kind) break the Zen of Python.

### "There should be one - and preferably only one - obvious way to do it."

Python developers hold this principle dear, and for good reason. Yet when it comes to something as basic as rendering "Hello, world!" in Django, the options are dizzying.
...
All this choice leads to inconsistency across projects and even within a single codebase, and new developers are forced to learn five ways to do one thing. Standardize on a single pattern, like the base `View` class, and this whole problem goes away.

### "Flat is better than nested."

Have you ever tried to understand where the magic in a generic `DetailView` comes from? You might need a map. The inheritance tree for even the most common generic views is deep and tangled. A `DetailView` inherits from `SingleObjectTemplateResponseMixin`, `BaseDetailView`, which inherits from `SingleObjectMixin`, `ContextMixin`, and finally the base `View`.

This nesting is the exact opposite of "flat". Logic is scattered across multiple parent classes, which makes it hard to track down where behavior comes from. Contrast this with the base `View`: all its methods (`get()`, `post()`, `setup()`, `dispatch()`) are at the same level. The logic is right there in front of you, not hidden five levels deep in an inheritance tree.
...
This makes debugging views so much harder. Did I forget to add `user_profile` to my context dictionary in the `get_context_data` method? Did I misspell it as `user_pofile` in the view? Or did I misspell it in the template? The silent failure means you have to debug in three places at once. So much for a principle designed to make debugging _easier_.

### "Practicality beats purity."

The Django template language was designed with the "purity" of separating logic from presentation in mind. That sounds nice in theory, but in practice it has led to some deeply impractical limitations.
...
The template language is intentionally crippled to enforce a philosophical ideal, and we're all left with a less powerful, more frustrating tool.

## Back to the Zen

Django is a wonderful framework, but the generic class-based view system, in its attempt to be a one-size-fits-all solution, often creates more complexity than it saves.

The base `View` class offers a more Pythonic path. It's simple, not complex. It's flat, not nested. Its implementation is easy to explain.
