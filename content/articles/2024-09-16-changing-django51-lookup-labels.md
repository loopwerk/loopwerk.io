...
For example let’s say your `User` models has a one to one relationship with an `AccountSettings` models, and in the admin’s list of users you want to show the user’s names as well as the value of `AccountSettings.pace_account_id`. Until Django 5.1 you’d have to create a getter function like this:
