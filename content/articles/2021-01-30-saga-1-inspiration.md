...
I didn't want to be forced to add that `-------` separator after the metadata. The solution was pretty simple: I wrote my own `MarkdownReader` class, added that in `settings.py` to be used, and it works. It was _very_ easy to adjust the way liquidluck converts articles and their metadata.
...
3. Use liquidluck's concept of multiple readers and writers, but they have to operate on sets of pages, so that you can write a list of articles _and_ a list of questions or apps.
