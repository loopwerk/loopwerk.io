...
Or perhaps you're using segues with storyboard:
...
Another example of coupling is when view controllers are not coupled to other view controllers, but to the way they're being presented. In the tiny example app we assume that the `DetailViewController` will always be pushed, but what if we want to sometimes show it modally? For example in another flow of the app, or on iPad. Now that view controller has to know if it's shown modally or not, so it can show a "close" button in the navigation bar for example.
...
Of course the coordinator as shown above is extremely simplistic, but it should hopefully illustrate the very basic concept of what coordinators are responsible for and how they can decouple view controllers. Simply said, every time you instantiate or configure a view controller within a different view controller, that's moved to the Coordinator. Every time you would directly call `pushViewController` or `present` from within a view controller, that's moved to the Coordinator. 
