...
```javascript title="/routes/auth/login.js"
...
export async function post({ request }) {
  ...
  try {
    body = (
      <mark title="This calls the external API">
        await postApi(fetch, "auth/login", await request.json())
      </mark>
    );
  } catch (error) {
    return {
...
```javascript title="/routes/auth/logout.js"
