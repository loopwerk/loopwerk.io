...
exports.appleWebhook = functions
  .runWith(runtimeOpts)
  .https.onRequest(async (req, res) => {
    ...
    const userQuerySnapshot = await db
      .collection("users")
      .where(
        "appleSubscription.originalTransactionId",
        "==",
    ...
    // Update the user
    return userQuerySnapshot.docs[0].ref
      .update({ status: status, appleSubscription: appleSubscription })
      .then(function () {
