...
function createStripeSession(priceId) {
  return functions
    .httpsCallable("createStripeSession")({ priceId: priceId })
    .catch(error => {
  ...
  .https.onCall(async (data, context) => {
    ...
    if (user.webSubscription && user.webSubscription.customer) {
      ...
      await userSnapshot.ref.update({
        webSubscription: { customer: customerId },
      });
    }
...
async function updateSubscription(subscription) {
  ...
  const userQuerySnapshot = await db
    .collection("users")
    .where("webSubscription.customer", "==", subscription.customer)
    .limit(1)
    .get();
  ...
  return userQuerySnapshot.docs[0].ref.update({
    status: status,
...
```javascript
exports.getStripePortalUrl = functions
  .runWith(runtimeOpts)
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
